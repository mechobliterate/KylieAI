class Dot {
  constructor(t1, t2, velX) {
    this.position = createVector(
      t1.pixelPos.x + tileSize / 2,
      t1.pixelPos.y + tileSize / 2,
    );
    ...
    for (var i = 0; i < this.bouncers.length; i++) {
      if (
        ...
        dist(
          this.position.x,
          this.position.y,
          this.bouncers[i].pixelPos.x + tileSize / 2,
          this.bouncers[i].pixelPos.y + tileSize / 2,
        ) < this.speed
      ) {
        this.bounceTimer = 10;
        this.bounceWait = 1;
  ...
  collides(ptl, pbr) {
    ...
    if (
      ...
      if (
        ...
        this.diameter / 2 + sqrt(playerSize * playerSize * 2) / 2
      ) {
        return true;
      }
    }
